<#
    .DESCRIPTION
        A runbook that will scale down and reprovision the Virtual Machine Scale Set instances.
		
		NOTE: The existing instances will be deleted and a new instances will be brought online with the
			  smaller size. If you'd like to avoid this behavior, please use the other VM Scale Set vertical
			  scale runbooks in the gallery.
...
if ($WebhookData -ne $null) {  
	...
	if ($WebhookBody.status -eq "Activated") {
		...
		$currentVmssCapacity = $vmss.Sku.Capacity
		...
		if($newVmssSize -eq $noResize -or [string]::IsNullOrEmpty($newVmssSize)) {
			...
			$vmss.Sku.Capacity = $currentVmssCapacity
			Update-AzureRmVmss -ResourceGroupName $ResourceGroupName -Name $VmssName -VirtualMachineScaleSet $vmss
